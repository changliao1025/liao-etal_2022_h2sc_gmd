...
sFilename_out = 'wtd_hillslope_month.png'

fig = plt.figure(dpi=iDPI, figsize=(iSize_x, iSize_y))

axgr = AxesGrid(fig, 111, nrows_ncols=(5, 2), axes_pad=0.1, label_mode='L') # note the empty label_mode
...
for iax, ax in enumerate(axgr):
    ...
    aY_all = list()
    aX_all.append(x00)
    aY_all.append(y00) #land surface
    ...
    if iCase_index == 1:
        ax.axis('off')
        continue
    x = [0, aHillslope_length[iCase_index-2]]
    aParameter_case  = pye3sm_read_case_configuration_file(sFilename_case_configuration,
                                                           iCase_index_in =  iCase_index ,
                                                           iYear_start_in = iYear_start,
                                                           iYear_end_in = iYear_end,
    ...
    for iMonth in range( 1, 13, 1):
        month_index = np.arange(iMonth-1, nstress_subset, 12)
        ...
        y1=y0+ dSlope_wt * aHillslope_length[iCase_index-2]
        y = [y0, y1]
        aX_all.append(x)
        aY_all.append(y)
        aLabel_legend.append(calendar.month_abbr[iMonth])
    ...
    #sTitle = 'Water table along the hillslope in Case 3'
    ...
    #  fontsize=8,  ncol=5)
    ax.text(0.1, 0.9, 'Case ' + str(iCase_index), ha='center', va='center', transform=ax.transAxes, fontsize=9)


# Collect handles and labels from the last ax
handles, labels = axgr[-1].get_legend_handles_labels()

# Separate handles and labels into two groups
group1_handles = handles[:3]
group1_labels = labels[:3]
group2_handles = handles[3:]
group2_labels = labels[3:]

# Create the legend on the first ax using the collected handles and labels
legend1 = axgr[0].legend(group1_handles, group1_labels, loc='upper left', fontsize=9, ncol=1 )
legend2 = axgr[0].legend(group2_handles, group2_labels, loc='lower left', fontsize=9, ncol=3)

# Ensure both legends are displayed
axgr[0].add_artist(legend1)
axgr[0].add_artist(legend2)
